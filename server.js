...
// ===== Members Single Export =====
app.post('/export-single-member', async (req, res) => {
  try {
    const { group, offset } = req.body;
    ...
    const participants = await client.getParticipants(entity, { limit:1, offset });
    if(!participants.length) return res.json({done:true});

    const p = participants[0];
    if(p.bot) return res.json({done:false, skipped:true});

    res.json({
      done:false,
      user:{user_id:p.id, username:p.username, access_hash:p.access_hash}
    });
  } catch(err){
...
app.post('/check-member', async(req,res)=>{
  try{
    ...
    }catch(e){return res.json({exists:false});}
  } catch(err){
    res.json({error:err.message});
  }
});
...
app.post('/add-member',async(req,res)=>{
  try{
    ...
    try{
      ...
      status="success"
      reason="joined"
      await sleep(20000) // 20s delay after success
    }catch(err){
      const wait=parseFlood(err)
      if(wait){
...
app.get('/account-status',async(req,res)=>{
  ...
  for(const id in data){
    const a=data[id]
    if(a.floodWaitUntil && a.floodWaitUntil-now<=0){
      a.status="active"
