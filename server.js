...
async function checkTGAccount(account){
  try{
    ...
    await client.getMe()
    account.status="active"
    await update(ref(db, `accounts/${account.id}`), {status:"active", lastChecked:Date.now(), floodWaitUntil:null})
  }catch(err){
    ...
    await update(ref(db, `accounts/${account.id}`), {status,error:err.message,lastChecked:Date.now(),floodWaitUntil:floodUntil})
  }
}
...
// ===== Scrape Members Live =====
app.post('/members-live', async(req,res)=>{
  try{
    const {group} = req.body
    if(!group) return res.json({error:"Missing group"})
    const safeKey = group.replace(/[^a-zA-Z0-9_-]/g,'')
    ...
    await set(ref(db, `members-live/${safeKey}`), members)
    res.json({success:true, count:members.length})
  }catch(err){ res.json({error:err.message}) }
})

// ===== Export Members =====
app.get('/export/:group', async(req,res)=>{
  try{
    const group = req.params.group
    const safeKey = group.replace(/[^a-zA-Z0-9_-]/g,'')
    const snap = await get(ref(db, `members-live/${safeKey}`))
    const members = snap.val()||[]
    res.setHeader('Content-Disposition', `attachment; filename=${safeKey}-members.json`)
    res.json(members)
...
app.post('/add-member', async(req,res)=>{
  try{
    ...
    if(!username && (!user_id || !access_hash)) return res.json({status:"skipped",reason:"missing username/access_hash",accountUsed:"none"})
...
app.get('/account-status',async(req,res)=>{
  ...
  for(const id in data){
    ...
    if(a.floodWaitUntil){
      ...
      }else{ a.remaining=remain; a.readyTime=new Date(a.floodWaitUntil).toLocaleString() }
    }
  }
  res.json(data)
})
