...
function randomDelay(min=30,max=40){ return Math.floor(Math.random()*(max-min+1)+min)*1000 }
...
app.post('/members', async (req,res)=>{
  try{
    ...
    const members = all.filter(p=>!p.bot).map(p=>({
      user_id:p.id,
      username:p.username,
      avatar:p.photo ? `https://t.me/i/userpic/320/${p.id}.jpg` : '/default-avatar.png' // fallback
    }))
    res.json(members)
...
app.post('/add-member', async(req,res)=>{
  try{
    ...
    if(!activeAccounts.length){
      await set(ref(db,'system_status'),{bannerLocked:true})
      return res.json({status:"failed", reason:"All accounts FloodWait", accountUsed:"none"})
    }
    await set(ref(db,'system_status'),{bannerLocked:false})
    ...
    try{
      ...
      const delay=randomDelay(30,40)
      console.log(`⏱ Waiting ${delay/1000}s before next add`)
      await sleep(delay)
    }catch(err){
      const wait=parseFlood(err)
      if(wait){
