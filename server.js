...
app.post('/members', async(req,res)=>{
  try{
    ...
    // Filter: not bot + last seen < 7 days
    const now = Date.now()
    const members = all.filter(p => !p.bot && (!p.status || 
      ((p.status.wasOnline||p.status.expires||0) > now - 7*24*3600*1000)
    )).map(p=>({
      user_id: p.id,
      username: p.username,
...
app.post('/add-member', async(req,res)=>{
  try{
    ...
    // ===== Check already in target group (real API) =====
